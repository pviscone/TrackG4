...
int main() {
    ...
    for (int i = 0; i < tree->GetEntries(); i++) {
        ...
        if ((event->detectorData.posX).empty()) {
            ...
            if ((fitdata.status) || (abs(fitdata.x0) > GeometryParameters::worldSize) || (abs(fitdata.y0) > GeometryParameters::worldSize) || (abs(fitdata.x0_err) > GeometryParameters::worldSize) || (abs(fitdata.y0_err) > GeometryParameters::worldSize) || (std::isnan(fitdata.x0)) || (std::isnan(fitdata.y0)) || (std::isnan(fitdata.mx)) || (std::isnan(fitdata.my))) {
                continue;
            }
